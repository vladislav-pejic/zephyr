...
struct adc_read_config {
	...
	struct adc_stream_trigger *const triggers;
};
...
struct adc_decoder_api {
	 ...
	 */
	int (*get_size_info)(uint32_t channel, size_t *base_size,
			     size_t *frame_size);
 ...
 */
#define ADC_DECODER_NAME() UTIL_CAT(DT_DRV_COMPAT, __decoder_api)

/**
 * @brief Statically get the decoder for a given node
 *
 * @code{.c}
 * static const adc_decoder_api *decoder = ADC_DECODER_DT_GET(DT_ALIAS(accel));
 * @endcode
 */
...
#define ADC_DT_STREAM_IODEV(name, dt_node, ...)                                                 \
